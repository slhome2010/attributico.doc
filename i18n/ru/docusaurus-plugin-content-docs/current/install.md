...
4\. В административной панели в разделе *Система ⇒ Пользователи ⇒ Группы пользователей* установите разрешения на просмотр и редактирование для `module/attributico` или `extension/module/attributico` для магазинов версии >2.2 . Установите разрешения на просмотр и редактирование для `attributico/interlink` и `attributico/unit`.

5\. В административной панели в разделе *Расширения ⇒ Установка расширений* загрузите файлы `attributico.3.x.ocmod.zip` для магазинов третьей версии (3.x) из папки `ocmod`.

Для тех, кто кто использует шаблон OCTEMPLATE, не выполняйте этот пункт. Описание установки модификаторов смотрите ниже.

6\. В административной панели в разделе *Расширения ⇒ Модификаторы* необходимо обновить модификаторы.
...
Не выполняйте пункт 5\. из инструкции по установке. Для магазинов версии 3.0.x необходимо из архива `attributico.3.x.ocmod.zip` извлечь файл `install.xml`, переименовать его в `attributico.3.ocmod.xml` и после этого скопировать его в папку `system`.
:::

## Обновление

1\. После скачивания новой версии надо распаковать архив и содержимое папки `upload.../`, соответствующей вашей версии магазина из архива скопировать поверх структуры папок сайта.
...
3\. При переходе на новую версию, возможно, придется деактивировать (деинсталировать) модуль и сразу активировать его заново в разделе *Расширения ⇒ Модули*. Эта операция создаст дополнительные таблицы в БД и новые обработчики событий, если они появились.

4\. Очистите [кэш магазина](https://shopiweb.ru/opencart-3/kesh-opencart-3-ochistka-kesha/) и браузера.

5\. Отредактируйте и сохраните заново настройки модуля.
...
В процессе установки модуля в базе данных будут созданы дополнительные [таблицы](technical-specifications/database.md).

:::tip
В процессе удаления модуля никакие таблицы и поля не удаляются. Если это для вас критично, то удалите их самостоятельно, например, с помощью PhpMyAdmin.
:::
